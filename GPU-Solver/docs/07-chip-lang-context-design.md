...
status: 🟢 칩 가드 구현됨 (회귀 0) / watch 배선 미완
---
...
**미완 (다음):**
- `harness`가 ctx를 `match`에 전달 안 함 (아직 ctx=None). 칩 탐지 = watch가
  `torch.cuda.get_device_capability`로 채워 mailbox로 넘기는 배선 필요 = **Colab측, 재시작 1회.**
