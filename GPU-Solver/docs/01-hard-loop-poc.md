...
### R6 — TF32 후 재프로파일 (천장 재편) + bf16 반증

TF32로 천장 깎았으니 재프로파일 (= 루프 정상 동작 확인: 천장 깎으면 다음 천장 드러남). 총 시간 1904→1133μs (matmul 1003→229μs, **-77%** TF32 효과 확정).

| 커널 | TF32 후 | TF32 전 |
|---|---|---|
| **flash_attn** | **48.6%** ★ (새 1위) | 28.7% |
| matmul (sgemm) | 20.3% ★ | 52.7% |
| elementwise (rope/resid) | 12.9% ★ | 7.7% |
| mul | 11.0% ★ | 6.5% |
| rmsnorm/reduce | 4.6% | 2.8% |

**천장 1위 = flash_attn(48.6%)으로 교체.** matmul이 TF32로 절반 떨어지며 attention이 올라옴. 주목: FFN mul이 6.5%→11%로 **게이트(5%) 통과** = R3' 융합이 이제 유효 타깃 (= 룰 재평가 실물, 환경 바뀌니 전 "헛수고"가 유효해짐).

**R6 가설(룰 발화)**: "flash_attn 48.6%+matmul 20.3%=69% 둘 다 텐서코어 op. bf16면 더 빠름." 변형 = 입력 dtype bf16.

| 변형 | latency | maxdiff | 정확성 |
|---|---|---|---|
| TF32 (현 챔피언) | 0.853 ms | 3.7e-4 | atol 1e-3 PASS |
| bf16 | 0.850 ms | **1.0e-2** | atol 1e-3 **FAIL** (2e-2만 PASS) |

**판정: bf16 = 네번째 반증.** TF32 대비 1.001×(동률). 속도 이득 0인데 **정확성만 10배 악화**(3.7e-4→1.0e-2, atol 1e-3 깨짐). 원인 = TF32가 이미 matmul/flash를 텐서코어로 태움. A100서 TF32 텐서코어 throughput ≈ bf16, flash는 둘 다 텐서코어 경로 → bf16 추가 가속 없음. **TF32 = 정확성-속도 스윗스팟.** 룰 "낮은 정밀도=빠름"이 이미 텐서코어 쓰는 상황선 안 통함 (측정이 기각).

**R6 결론: TF32 챔피언 유지 (0.84~0.85ms, 누적 3.86×).** bf16 미승격. 남은 천장(flash 48.6%·elementwise 12.9%·mul 11%)은 dtype 아닌 융합/알고리즘 변형 필요 = 다음 세션.
...
- [x] TF32 후 재프로파일: 천장 재편 flash_attn 48.6%(신1위)/matmul 20.3%/elementwise 12.9%/mul 11%. §R6.
- [x] R6 bf16 = 네번째 반증. TF32 대비 1.001×(동률), 정확성만 10배 악화. TF32 챔피언 유지(스윗스팟).
- [x] fused_ffn 승격 시도 → **측정 기각.** TF32서 median flash4d 0.857 vs fused_ffn 0.933 = **0.92×(8% 느림).** mul 비중 11%(게이트 통과)지만 승격하면 회귀. 원인 = TF32로 matmul 빨라지며 전체 단축 → Triton launch+동기화 오버헤드가 상대적으로 커짐. **룰 정밀화: "게이트 통과 = 시도 가치" ≠ "시도 = 승격". 실측이 최종 심판.** flash4d 챔피언 유지, fused_ffn은 variant 보존.
- [x] R7 elementwise 분해 → **착수 전 중단(룰 선험적 기각).** ncu 세분: elementwise 12.9% 중 setup 커널(normal/uniform=`_make_case` 가중치, arange/cos/sin=rope 테이블)이 비중 부풀림 — forward 본체 아님. 실제 forward = rope 곱(~20μs) 지배, residual add(3.4μs) 미미. **rope 곱 ≈ mul(11%) 크기.** R6서 mul 융합이 TF32서 8% 회귀 입증됨 → 같은 launch+동기화 오버헤드 함정. 비슷한 크기 커널 융합 = 동일 결과 예측. **룰 적용: "TF32 환경 작은 elementwise 융합 = launch 오버헤드에 먹힘" (R6 학습을 R7에 선험 적용, 헛 라운드 절약).**
- [ ] R8 후보: 유일하게 의미있는 잔여 천장 = **flash_attn 48.6%**(근본 attention 비용). seq별 블록 튜닝/다른 attention 변형 = 알고리즘 영역, 난이도↑. 또는 여기서 종료(naive→3.86× 충분한 곡선).
