...
차별점 = **세 축 각각 입증: gain 축=matmul 6.4×(A100), 진화 축=sigmoid 오발화 retire(A100), 칩 축=T4서 TF32 헛가설 가드 차단(2026-06-30). 다문제 동시·일반화는 future work.**
...
0. **(칩 이식성 — ✅ 칩 가드 + ctx 배선 + T4 실측 완료 2026-06-30)** [[07-chip-lang-context-design]]. 칩·문법이 룰 cond에 1급 변수 아님(rationale 주석에만 A100·Triton) = matmul_tri/llama 난항 일부 원인. 설계+구현 확정: **칩=1급**(CHIP_CAPS 박힌 사실, 룰 cond에 `ctx.cap` 가드), **문법=신호 흡수**(Mojo 환경 0이라 1급 안 만듦). 황금규칙=**사용자/LLM은 Context(무대)만 설정, 최적 룰은 측정→진화가 발견**(룰 손수정 금지=차별점 보존).
   ...
   - ⚠️ **reg_pressure=헛가설** (cond `occ<0.5&reg>64` 맞으나 compute 포화라 무효). **retire 미관찰** — `CONVERGE_PATIENCE=3`==`RETIRE_AFTER_FAILS=3` 충돌로 수렴이 retire보다 먼저(5R converged). patience>fail 튜닝하면 보이나 infra변경=재시작. retire는 sigmoid서 이미 입증 → T4 재입증 불요.
