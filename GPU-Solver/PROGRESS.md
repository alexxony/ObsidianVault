...
남은 핵심 = 성능 gain (진화→더 빠른 커널). sigmoid 시도 → null(여지 없는 메모리바운드). 여지 큰 문제 필요.
...
- **성능 gain** — "진화→더 빠른 커널". sigmoid `--latency` 12R 시도 → **null (예상 적중)**:
  메모리바운드라 BLOCK 변형해도 latency ~770ms 동일(±1%) = 최적화 여지 없음. 룰 진화는
  재현되나 더 빠른 커널로 안 이어짐 = **루프 결함 아니라 문제 선택 문제.**
  → **여지 큰 문제(groupnorm/llama)** + **진짜 다른 커널**(BLOCK 1줄 아님)로 재시도 필요.
- **다문제 일반화** — gain 신호는 sigmoid 1문제만.
- 차별점 현 상태 = **"개념·메커니즘·룰진화 이득 OK, 최종 성능 gain 미입증"** (정직 유지).

## 🔖 다음 할 일 = 성능 gain (여지 큰 문제)

1. **groupnorm**(reduction 여지) 또는 llama(matmul 여지)로 재시도. 메모리바운드 sigmoid는 여지 없어 부적합.
2. variant = BLOCK 1줄 치환 아니라 **진짜 다른 알고리즘 커널** (대행, cherry-pick 안 되게 발화 가설 충실).
3. `python3 run_gain_compare.py groupnorm <variants_dir> 6 --latency` (ON/OFF + 성능 gain 자동 판정. 인프라 준비됨).
- ⚠️ llama=multi-kernel ncu 10~12분/R = 느림. groupnorm 먼저.
