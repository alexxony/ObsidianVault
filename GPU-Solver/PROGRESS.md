...
- **성능 gain** — "진화→더 빠른 커널". **3문제 전부 null (실측 확정):**
  ...
  | matmul (큰 4096² fp32) | TF32 OFF vs ON, 순수 compute-bound | null — **OFF=95040us, ON=95232us (1.00×)** |
  ...
  - ✅ **(2026-06-29) 순수 compute-bound matmul로 환경 한계 재확정** — llama가 attention 지배라 matmul 안 보인 것 의심해
    **큰 4096² fp32 matmul**(`problems/matmul/`, FLOP=O(N³)≫mem) 추가, TF32 OFF/ON 직접 비교. 결과 **95040us vs 95232us = 1.00× 동률.**
    **결정적 단서: OFF(`allow_tf32=False`)인데도 ncu tc=True** → 전역 플래그가 watch executor서 무효(cuBLAS 핸들 선초기화 추정)거나
    ncu 측정값이 TF32 설정 무관. = **이 측정 환경(git-mailbox Colab executor 경로)이 TF32 speedup 자체를 못 보여줌.**
    순수 compute 문제로도 재현 → **환경이 진짜 블로커, 문제 선택으로 못 우회** 확정.
  - = **루프 결함 아님. 어떤 문제도 이 측정 환경서 최적화 여지 안 보임** (환경이 TF32 효과 못 냄, 문제 선택 무관).
