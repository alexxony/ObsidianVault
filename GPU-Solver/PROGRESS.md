...
0. **(칩 이식성 — ✅ 칩 가드 + ctx 배선 + T4 실측 완료 2026-06-30)** [[07-chip-lang-context-design]]. 칩·문법이 룰 cond에 1급 변수 아님(rationale 주석에만 A100·Triton) = matmul_tri/llama 난항 일부 원인. 설계+구현 확정: **칩=1급**(CHIP_CAPS 박힌 사실, 룰 cond에 `ctx.cap` 가드), **문법=신호 흡수**(Mojo 환경 0이라 1급 안 만듦). 황금규칙=**사용자/LLM은 Context(무대)만 설정, 최적 룰은 측정→진화가 발견**(룰 손수정 금지=차별점 보존).
   ...
   - ✅ **칩 자동탐지 배선 완료 (2026-07-01 후속)** — `--chip` 생략 시 canary raw_script 1발이 Colab GPU서 `get_device_capability`→`detect_chip` 실행→RES `chip` 필드→`Context` 자동 채움. 수동 `--chip`은 override 보존, 탐지실패→"" 가드통과(회귀 0). 3파일: `executor._detect_chip_now`+RES chip, `mailbox.submit_raw`(raw_script 왕복), `run_gain_compare._detect_chip_via_mailbox`. self-check 3개 PASS. ⬜ **남음=실 Colab 왕복 검증** (self-check는 fake responder까지, watch 살아있으면 `run_gain_compare matmul ... --latency` --chip 없이 → `[자동탐지(canary)]` 로그 확인).
