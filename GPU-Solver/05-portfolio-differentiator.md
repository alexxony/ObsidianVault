...
- ⚠️ **두 축 분리:** loop improves(gain, matmul ✅) + evolution beats static(진화, sigmoid ✅) =
  ...
  - **한 문제 동시 = 구조적 난제로 확인(측정).** 비합착 Triton matmul로 "틀린룰 발화→retire→맞는룰→gain"
    노렸으나, 시드 룰이 잘 맞아 첫 발화가 항상 맞는 룰 → retire 안 생김. **틀린 룰 자연 발화 = 메모리바운드
    (gain 천장) vs 맞는 룰 = compute(retire 불요) = 상호배타.** 연출 없이 한 문제서 둘 다 = 불가. = 한계도
    측정으로 밝힌 정직 기록.
