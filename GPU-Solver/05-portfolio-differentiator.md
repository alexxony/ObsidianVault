...
그 진화 메커니즘을 **통제된 ON/OFF 실험으로 입증**했다 (sigmoid+conv 2문제, A100).
성능 이득은 **matmul서 6.4× 실측**(fp32→TF32, compute-bound). 메모리바운드·occupancy포화 문제선
gain=null = 물리 천장(루프 결함 아님). gain 다문제 일반화는 future work로 정직히 명시.
...
### 5. 환경(칩)을 룰의 1급 입력으로 — 칩 가드 (✅ 2026-06-30, 진짜 T4 실측)

같은 신호라도 **최적 처방은 칩에 의존.** fp32 matmul "느림" 신호 → A100=TF32 켜라, T4=TF32 없음.
이 환경 의존성을 룰 cond에 1급 가드(`chip_cap`)로 넣되, **룰 내용은 사람이 안 짜고 측정→진화가 발견** (황금규칙).

| 칩 | fp32 matmul 신호 → 발화 룰 | 결과 |
|---|---|---|
| A100 (sm_80) | `fp32_no_tensorcore` 발화 | TF32 → 6.4× gain |
| **T4 (sm_75)** | 가드(`chip_cap="tf32"`) **차단** → `reg_pressure` | 헛가설(TF32) 사전 차단 |

**칩만 바꿔(Context) 발화 룰이 갈림 — 룰 손 안 댐.** = "사용자는 무대(환경)만 설정, 최적 룰은 측정이 발견"
실증. CHIP_CAPS(NVIDIA 공개 스펙)가 헛가설을 거름. 선행(CUDAMaster/CudaForge)은 CUDA+특정 GPU 고정 →
**칩 이식성 = 진짜 빈 자리.**

**이중 방어:** 칩 가드 = *알려진* 헛가설 사전 차단(선언적) + 진화 retire = *모르는* 헛가설 사후 폐기(측정).

> ⚠️ **정직:** T4 reg_pressure는 cond(`occ<0.5 & reg>64`)는 맞으나 compute 포화(96.7%)라 무효=헛가설.
> 진화가 잡아야 할 케이스지만 `CONVERGE_PATIENCE=3`==`RETIRE_AFTER_FAILS=3` 충돌로 retire 전 수렴종료 →
> T4 retire는 미관찰(retire 자체는 sigmoid서 입증됨). 측정 1칸(T4)만 주장, N칸 일반화는 미입증.
...
- ⚠️ **다문제 일반화 — 진화 축은 ✅ 다문제, gain 축은 아직 1문제.**
  - **진화 축 = sigmoid + conv2d 2문제** (2026-07-01 conv 추가). conv서도 `fp32_no_tensorcore` 헛발화
    자연 발생 → ON=retire 2회→uncoalesced 전환, OFF=영원(A100 8R). **retire 메커니즘 2문제 재현.**
  - **gain 축 = matmul 1문제** (conv gain=null, compute 천장). gain 다문제는 미입증.
- ⚠️ **두 축 분리:** loop improves(gain, matmul ✅) + evolution beats static(진화, sigmoid+conv ✅ 2문제) =
  각각 실증(진화는 다문제). 한 문제서 둘 다 = future work.
  - **한 문제 동시 = 구조적 난제로 확인(측정, 2번 재확인).** ① 비합착 Triton matmul로 "틀린룰→retire→맞는룰→gain"
    노렸으나 시드 룰이 잘 맞아 첫 발화가 항상 맞는 룰 → retire 안 생김. ② **conv2d서 틀린룰 retire는 됐으나
    그 후 맞는룰(uncoalesced) gain=null**(합착 variant 0.79×, occupancy 포화라 개선 무효). **틀린 룰 자연 발화 =
    gain 천장 문제 vs 맞는 룰 = retire 불요 = 상호배타.** conv이 "retire까진 갔으나 gain 벽" 재확인 = 정직 기록.
