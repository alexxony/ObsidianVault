...
| [[04-multiproblem-round-design]] | 다문제 라운드 (A 발화관찰→B 진화) — 차별점 실험 | 🟢 A PASS + B 대행 GPU 닫힘 실증 / gain layer(3문제 N라운드) 남음 |
...
- ⏭️ 다음 — **차별점 실험이 급소 (인프라 깔았으니 이제 진짜 목적)**:
  - **(핵심) 다문제 우수성 실험** — 여러 다른 GPU 커널(다른 병목 분포)로 "추가 축(비중·텐서코어·진화)이 측정 이득"을 객관 GT로 증명/반증. 이게 프로젝트 존재 이유 — 안 하면 영구히 "개념 OK, 이득 미입증". **GPU 다회 필수(큰 작업)**. 우편함으로 이제 현실화됨. 선결: 커널셋 N개 정의 + GT 정의 + 측정 프로토콜.
  - (전제) **`execute_request` Colab 채움** — challenge reference_impl gate + ncu 프로파일. 이게 되면 **첫 자동 라운드 = 배관 실증(1문제)**. 우편함 repo 1개 생성(GitHub 계정 필요) + Colab Secrets에 PAT.
  - (보조) glue.py Generator/Gate Real화. evolver propose_candidate 진짜화는 우수성 입증 후.
