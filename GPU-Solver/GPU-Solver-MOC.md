...
- ✅ **환경 한계 2중 확정 (2026-06-29)** — (1) llama ncu 비중: attention(flash) 54% > matmul 23%, flash 단일 48% 지배 → matmul 비병목 확정. (2) 순수 큰 matmul(4096² fp32) TF32 OFF/ON: **95040 vs 95232us 동률(1.00×)**, OFF인데 ncu tc=True → **이 측정 환경이 TF32 speedup 자체를 못 냄.** 문제 선택으로 못 우회 = 환경이 진짜 블로커. gain=환경 갖춰지면 측정 가능(인프라 준비됨)으로 정직 유지. 인프라 부수: `executor.ncu_breakdown` 분기, `problems/matmul/`.
