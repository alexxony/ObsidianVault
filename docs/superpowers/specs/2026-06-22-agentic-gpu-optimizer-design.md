...
### 2차 방어선 (진화 보조, 미검증) — roofline coarseness 극복

기하/임계값 roofline 라벨은 latency·occupancy·L2-thrash를 'memory bound'로 **오판**(Nsight 한계, VI-HPS/NERSC 경고). 진화 룰DB가 측정 트레이스로 **근본원인까지 세분**하면 진화의 강력한 응용. **단 미검증 — PoC가 진화 먼저 입증, coarseness는 후속.**

### 리스크 (정직)

차별점이 **진화 메타루프 하나에 전부 걸림.** 작동 못 하면 = CUDAMaster 열등복제. **"진화 룰 > 정적 임계값" 입증이 PoC 핵심 과제** (현재 미증명, [[01-hard-loop-poc]] 다음 단계).
